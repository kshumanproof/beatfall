...
/* THE ONE LINE TO CHANGE WHEN beatfall.app IS POINTED AT VERCEL.

   It was 'https://beatfall.app' and that domain is bought but not pointed, so
   it served nothing and FIVE things in every sign-in email were broken at once:
   the wordmark, its dark-mode twin, and the Help, Privacy and Terms links in
   the footer. Kris saw it as a missing logo, which is the only one of the five
   that shows. An email cannot carry its own images (a data URI is refused by
   Gmail and by Outlook), so this host has to be one that actually answers.

   The path shapes below are right either way: `cleanUrls` is on in
   vercel.json, so /help, /privacy and /terms resolve without .html.

   When the domain is live: put it back to 'https://beatfall.app', run
   `node build.js`, and paste BOTH templates into Supabase again. Nothing in
   email/ deploys; pasting is the only way they ship. */
const SITE    = 'https://beatfall-beta.vercel.app';
...
<style>
  ...
  @media (prefers-color-scheme: dark) {
    ...
    .bf-lockup  { content:url("${SITE}/brand/lockup-tagline-dark.png"); }
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="600"
              ...
              <tr>
                <td class="bf-pad" style="padding:22px 6px;">
                  ...
                  <img src="${SITE}/brand/lockup-tagline.png" alt="Beatfall: where your story falls into place." width="182" height="57"
...
/* AND THE MODULE THE SERVER ACTUALLY SENDS.

   This was a hand-kept second copy of the same markup, and it had already
   drifted: when the host moved off beatfall.app the two templates above
   followed and this one did not, so the one email that goes to somebody about
   to lose their account was still pointing at a domain that serves nothing.
   That is the same "change one, change all three" failure this repo has been
   bitten by before, so it is generated now rather than remembered.

   Two things keep it safe to write from here: the string is wrapped in
   backticks, so any backtick or ${'${'}} inside the HTML has to be escaped, and
   `api/cleanup.js` imports DELETION_WARNING_HTML by name. Do not rename it. */
const escaped = WARNING.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${');
fs.writeFileSync('../api/_email/deletion-warning.js',
  `/* The five-month warning, as HTML. GENERATED by email/build.js.\n`
+ ` * Do not edit this file: edit email/build.js and run \`node build.js\`.\n`
+ ` *\n`
...
+ ` */\nexport const DELETION_WARNING_HTML = \`${escaped}\`;\n`);
console.log('built ../api/_email/deletion-warning.js');
