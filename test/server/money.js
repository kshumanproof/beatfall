import { charge, refund, entitlement, spend } from './api/_lib/core.js';
...
{
  const db = makeDb(paid({credits_used:149, credits_extra:10}));
  const p = db.state.profile, e = entitlement(p);
  const r = await charge(db, 'u1', p, e, 2);
  check('a charge spanning both buckets applies', r.ok === true);
  check('the month is spent first',
    db.state.profile.credits_used === 150 && db.state.profile.credits_extra === 9,
...
// ---------- nothing left
{
  const db = makeDb(paid({credits_used:150, credits_extra:0}));
  const p = db.state.profile, e = entitlement(p);
  const r = await charge(db, 'u1', p, e, 1);
  check('an empty account is refused', r.ok === false && r.reason === 'insufficient', JSON.stringify(r));
  check('and nothing is taken', db.state.profile.credits_used === 150);
}
...
{
  const db = makeDb(paid({credits_used:149}));
  const results = [];
  for (let i = 0; i < 20; i++) {
  ...
  check('and the balance never goes past the allowance',
    db.state.profile.credits_used === 150, 'used=' + db.state.profile.credits_used);
}
...
// ---------- putting it back
{
  ...
  check('and each credit goes back where it came from',
    db.state.profile.credits_used === 149 && db.state.profile.credits_extra === 10,
